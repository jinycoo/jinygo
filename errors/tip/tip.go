...
var (
	...
	defaultConfig = &Config{
		Domain: "api.jinycoo.com",
		All:    ctime.Duration(time.Hour),
		Diff:   ctime.Duration(time.Minute * 5),
		ClientConfig: &jiny.ClientConfig{
			App: &jiny.App{
				Key:    "3c4e41f926e51656",
				Secret: "26a2095b60c24154521d24ae62b885bb",
			},
			Dial:    ctime.Duration(time.Second),
...
func Init(conf *Config) {
	...
	defualtEcodes.client = jiny.NewClient(conf.ClientConfig)
	...
	for {
		...
		if cur.Sub(last) > all {
			if ver, err = e.update(0); err != nil {
				log.Errorf("e.update() error(%v)", err)
			...
			if ver, err = e.update(lastVer); err != nil {
				log.Errorf("e.update(%d) error(%v)", lastVer, err)
				time.Sleep(10 * time.Second)
				continue
			}
		}
