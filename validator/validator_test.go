...
func TestStructLevelInvalidError(t *testing.T) {
	...
	fe := errs[0]
	assert.Equal(t, fe.Field(), "Value")
	assert.Equal(t, fe.StructField(), "Value")
	assert.Equal(t, fe.Namespace(), "StructLevelInvalidErr.Value")
	assert.Equal(t, fe.StructNamespace(), "StructLevelInvalidErr.Value")
	assert.Equal(t, fe.Tag(), "required")
	assert.Equal(t, fe.ActualTag(), "required")
	assert.Equal(t, fe.Kind(), reflect.Invalid)
	assert.Equal(t, fe.Type(), reflect.TypeOf(nil))
}
...
func TestNameNamespace(t *testing.T) {
	...
	assert.Equal(t, fe.Field(), "JSONString[1]")
	assert.Equal(t, fe.StructField(), "String[1]")
	assert.Equal(t, fe.Namespace(), "Namespace.Inner1JSON.Inner2JSON.JSONString[1]")
	assert.Equal(t, fe.StructNamespace(), "Namespace.Inner1.Inner2.String[1]")
}
...
func TestAnonymous(t *testing.T) {
	...
	assert.Equal(t, tst.anonymousC.c, "")
	...
	assert.NotEqual(t, fe, nil)
	assert.Equal(t, fe.Field(), "BEE")
	assert.Equal(t, fe.StructField(), "B")
...
func TestStructLevelReturnValidationErrorsWithJSON(t *testing.T) {
	...
	assert.Equal(t, fe.Field(), "JSONString")
	assert.Equal(t, fe.StructField(), "String")
	assert.Equal(t, fe.Namespace(), "TestStructReturnValidationErrors.Inner1JSON.Inner2.JSONString")
	assert.Equal(t, fe.StructNamespace(), "TestStructReturnValidationErrors.Inner1.Inner2.String")
	...
	assert.Equal(t, fe.Field(), "JSONString")
	assert.Equal(t, fe.StructField(), "String")
	assert.Equal(t, fe.Namespace(), "TestStructReturnValidationErrors.Inner1JSON.TestStructReturnValidationErrorsInner2.JSONString")
	assert.Equal(t, fe.StructNamespace(), "TestStructReturnValidationErrors.Inner1.TestStructReturnValidationErrorsInner2.String")
}
...
func TestAliasTags(t *testing.T) {
	...
	assert.NotEqual(t, fe, nil)
	assert.Equal(t, fe.ActualTag(), "hexcolor|rgb|rgba|hsl|hsla")
...
func TestStructPartial(t *testing.T) {
	...
	assert.Equal(t, len(errs.(ValidationErrors)), 1)
...
func TestCrossStructLteFieldValidation(t *testing.T) {
	...
	assert.NotEqual(t, errs, nil)
	assert.Equal(t, len(errs.(ValidationErrors)), 6)
	AssertError(t, errs, "Test.CreatedAt", "Test.CreatedAt", "CreatedAt", "CreatedAt", "ltecsfield")
	AssertError(t, errs, "Test.String", "Test.String", "String", "String", "ltecsfield")
	AssertError(t, errs, "Test.Int", "Test.Int", "Int", "Int", "ltecsfield")
	AssertError(t, errs, "Test.Uint", "Test.Uint", "Uint", "Uint", "ltecsfield")
...
func TestCrossNamespaceFieldValidation(t *testing.T) {
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, kind, reflect.Struct)
	...
	assert.Equal(t, current.String(), "val2")
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, kind, reflect.String)
	assert.Equal(t, current.String(), "val3")
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, kind, reflect.String)
	assert.Equal(t, current.String(), "val1")
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, kind, reflect.Ptr)
	assert.Equal(t, current.String(), "<*validator.SliceStruct Value>")
	assert.Equal(t, current.IsNil(), true)
...
func TestExistsValidation(t *testing.T) {
	...
	assert.Equal(t, err, nil)
	assert.NotEqual(t, ting, nil)
	assert.Equal(t, ting.Truthiness, nil)
	...
	assert.Equal(t, err, nil)
	assert.NotEqual(t, ting, nil)
	assert.Equal(t, ting.Truthiness, true)
...
func TestTCP4AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		if test.expected {
			...
			if assert.Nil(t, errs) {
				t.Log(test.param, assert.Nil(t, errs))
				t.Fatalf("Index: %d tcp4_addr failed Error: %s", i, errs)
			} else {
				val := getError(errs, "", "")
...
func TestInterfaceErrValidation(t *testing.T) {
	...
	assert.NotEqual(t, errs, nil)
	assert.Equal(t, len(errs.(ValidationErrors)), 4)
...
func TestMapDiveValidation(t *testing.T) {
	...
	assert.NotEqual(t, fe, nil)
	assert.Equal(t, fe.Tag(), "gte")
	assert.Equal(t, fe.ActualTag(), "gte")
	assert.Equal(t, fe.Field(), "MyName[Key2]")
	assert.Equal(t, fe.StructField(), "Map[Key2]")
}
...
func TestArrayDiveValidation(t *testing.T) {
	...
	assert.NotEqual(t, errs, nil)
	assert.Equal(t, len(errs.(ValidationErrors)), 5)
	...
	assert.NotEqual(t, errs, nil)
	assert.Equal(t, len(errs.(ValidationErrors)), 3)
...
func TestBase64URLValidation(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if tc.success {
			...
			assert.Equal(t, innerErr, nil)
			assert.Equal(t, tc.decoded, string(d))
		} else {
			assert.NotEqual(t, err, nil)
			if len(tc.encoded) > 0 {
...
func TestOrTag(t *testing.T) {
	...
	ve := errs.(ValidationErrors)
	assert.Equal(t, len(ve), 1)
	assert.Equal(t, ve[0].Tag(), "eq=|eq=blue")
...
func TestStructStringValidation(t *testing.T) {
	...
	// Assert Top Level
	assert.NotEqual(t, errs, nil)
	assert.Equal(t, len(errs.(ValidationErrors)), 13)
...
func TestStructInt32Validation(t *testing.T) {
	...
	// Assert Top Level
	assert.NotEqual(t, errs, nil)
	assert.Equal(t, len(errs.(ValidationErrors)), 10)
...
func TestStructSliceValidation(t *testing.T) {
	...
	assert.NotEqual(t, fe, nil)
	assert.Equal(t, fe.Field(), "Len")
	assert.Equal(t, fe.StructField(), "Len")
	assert.Equal(t, fe.Namespace(), "TestSlice.Len")
	assert.Equal(t, fe.StructNamespace(), "TestSlice.Len")
	assert.Equal(t, fe.Tag(), "len")
	assert.Equal(t, fe.ActualTag(), "len")
	assert.Equal(t, fe.Param(), "10")
	assert.Equal(t, fe.Kind(), reflect.Slice)
	assert.Equal(t, fe.Type(), reflect.TypeOf([]int{}))
...
func TestInvalidStruct(t *testing.T) {
	...
	assert.NotEqual(t, err, nil)
	assert.Equal(t, err.Error(), "validator: (nil)")
...
func TestCustomFieldName(t *testing.T) {
	...
	assert.Equal(t, getError(errs, "A.b", "A.B").Field(), "b")
	assert.Equal(t, getError(errs, "A.c", "A.C").Field(), "c")
	assert.Equal(t, getError(errs, "A.d", "A.D").Field(), "d")
	assert.Equal(t, getError(errs, "A.E", "A.E").Field(), "E")
...
func TestMutipleRecursiveExtractStructCache(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func() {
			<-proceed
			sc := validate.extractStructCache(current, name)
			assert.Equal(t, ptr, fmt.Sprintf("%p", sc))
		}()
	}
...
func TestTranslations(t *testing.T) {
	...
	fe := errs[0]
	assert.Equal(t, fe.Tag(), "required")
	assert.Equal(t, fe.Namespace(), "Test.Value")
	assert.Equal(t, fe.Translate(trans), fmt.Sprintf("%s is a required field", fe.Field()))
	assert.Equal(t, fe.Translate(fr), fmt.Sprintf("%s est un champ obligatoire", fe.Field()))

	nl := nl.New()
	uni2 := ut.New(nl, nl)
	trans2, _ := uni2.GetTranslator("nl")
	assert.Equal(t, fe.Translate(trans2), "Key: 'Test.Value' Error:Field validation for 'Value' failed on the 'required' tag")

	terrs := errs.Translate(trans)
	assert.Equal(t, len(terrs), 1)

	v, ok := terrs["Test.Value"]
	assert.Equal(t, ok, true)
	assert.Equal(t, v, fmt.Sprintf("%s is a required field", fe.Field()))
	...
	v, ok = terrs["Test.Value"]
	assert.Equal(t, ok, true)
	assert.Equal(t, v, fmt.Sprintf("%s est un champ obligatoire", fe.Field()))
	...
	fe = errs[0]
	assert.Equal(t, fe.Tag(), "gt")
	assert.Equal(t, fe.Namespace(), "Test2.Value")
	assert.Equal(t, fe.Translate(trans), "Key: 'Test2.Value' Error:Field validation for 'Value' failed on the 'gt' tag")
}
...
func TestTranslationErrors(t *testing.T) {
	...
	assert.NotEqual(t, err, nil)
	assert.Equal(t, err.Error(), "error: conflicting key 'required' rule 'Unknown' with text '{0} is a required field' for locale 'en', value being ignored")
}
...
func TestStructFiltered(t *testing.T) {
	...
	assert.NotEqual(t, err, nil)
	assert.Equal(t, err.Error(), "validator: (nil *time.Time)")
}
...
func TestFieldLevelName(t *testing.T) {
	...
	errs := validate.Struct(test)
	assert.Equal(t, errs, nil)
	assert.Equal(t, res1, "json1")
	assert.Equal(t, alt1, "String")
	assert.Equal(t, res2, "json2[0]")
	assert.Equal(t, alt2, "Array[0]")
	assert.Equal(t, res3, "json3[test]")
	assert.Equal(t, alt3, "Map[test]")
	assert.Equal(t, res4, "json4")
	assert.Equal(t, alt4, "Array2")
	assert.Equal(t, res5, "json5")
	assert.Equal(t, alt5, "Map2")
}
...
func TestValidateStructRegisterCtx(t *testing.T) {
	...
	assert.Equal(t, errs, nil)
	assert.Equal(t, ctxVal, "testval")
	assert.Equal(t, ctxSlVal, "slVal")
}
...
func TestIsDefault(t *testing.T) {
	...
	assert.Equal(t, fe.Field(), "Inner")
	assert.Equal(t, fe.Namespace(), "Test.Inner")
	assert.Equal(t, fe.Tag(), "isdefault")
	...
	fe = errs.(ValidationErrors)[0]
	assert.Equal(t, fe.Field(), "inner")
	assert.Equal(t, fe.Namespace(), "Test2.inner")
	assert.Equal(t, fe.Tag(), "isdefault")
}
...
func TestKeys(t *testing.T) {
	...
	err := validate.Struct(tst)
	assert.NotEqual(t, err, nil)
	assert.Equal(t, len(err.(ValidationErrors)), 3)
	...
	assert.NotEqual(t, err, nil)
	assert.Equal(t, len(err.(ValidationErrors)), 1)
...
func TestKeysCustomValidation(t *testing.T) {
	...
	assert.Equal(t, e.Param(), "")
	assert.Equal(t, e.Value().(LangCode), LangCode("xxx"))
...
func TestLookup(t *testing.T) {
	...
	assert.Equal(t, New().Struct(lookup), nil)
}
