...
func newFakeClient(b *testing.B) *es6.Client {
	cfg := es6.Config{Transport: &FakeTransport{RoundTripFn: defaultRoundTripFn}}
	es, err := es6.NewClient(cfg)
...
func newFakeClientWithError(b *testing.B) *es6.Client {
	cfg := es6.Config{Transport: &FakeTransport{RoundTripFn: errorRoundTripFn}}
	es, err := es6.NewClient(cfg)
