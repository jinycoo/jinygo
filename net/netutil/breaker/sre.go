...
func (b *sreBreaker) Allow() error {
	...
	//if log.V(5) {
	//	log.Infof("breaker: request: %d, succee: %d, fail: %d", total, success, total-success)
	//}
	...
	rr := b.r.Float64()
	//if log.V(5) {
	//	log.Infof("breaker: drop ratio: %f, real rand: %f, drop: %v", dr, rr, dr > rr)
	//}
	if dr <= rr {
		return nil
	}
...
func (b *sreBreaker) ReportResult(err error) {
	b.stat.Value()
}
