...
var _grpcTarget config.StringVars

var (
	_once           sync.Once
	_defaultCliConf = &ClientConfig{
		Dial:    ctime.Duration(time.Second * 10),
		Timeout: ctime.Duration(time.Millisecond * 250),
		Subset:  50,
	}
...
func (c *Client) SetConfig(conf *ClientConfig) (err error) {
	...
	if conf.Dial <= 0 {
		conf.Dial = ctime.Duration(time.Second * 10)
	}
	if conf.Timeout <= 0 {
		conf.Timeout = ctime.Duration(time.Millisecond * 250)
	}
	if conf.Subset <= 0 {
		conf.Subset = 50
	}
