...
var (
	...
	_defaultSerConf = &ServerConfig{
		Network:           "tcp",
		Addr:              "0.0.0.0:9000",
		Timeout:           ctime.Duration(time.Second),
		IdleTimeout:       ctime.Duration(time.Second * 60),
		MaxLifeTime:       ctime.Duration(time.Hour * 2),
		ForceCloseWait:    ctime.Duration(time.Second * 20),
		KeepAliveInterval: ctime.Duration(time.Second * 60),
		KeepAliveTimeout:  ctime.Duration(time.Second * 20),
	}
...
func (s *Server) SetConfig(conf *ServerConfig) (err error) {
	...
	if conf.Timeout <= 0 {
		conf.Timeout = ctime.Duration(time.Second)
	}
	if conf.IdleTimeout <= 0 {
		conf.IdleTimeout = ctime.Duration(time.Second * 60)
	}
	if conf.MaxLifeTime <= 0 {
		conf.MaxLifeTime = ctime.Duration(time.Hour * 2)
	}
	if conf.ForceCloseWait <= 0 {
		conf.ForceCloseWait = ctime.Duration(time.Second * 20)
	}
	if conf.KeepAliveInterval <= 0 {
		conf.KeepAliveInterval = ctime.Duration(time.Second * 60)
	}
	if conf.KeepAliveTimeout <= 0 {
		conf.KeepAliveTimeout = ctime.Duration(time.Second * 20)
	}
	if conf.Addr == "" {
