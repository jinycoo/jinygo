/**------------------------------------------------------------**
 * @filename middleware/prom.go.go
 * @author   jinycoo - caojingyin@baimaohui.net
 * @version  1.0.0
 * @date     2021/5/26 下午5:05
 * @desc     go.fofa.so - prom
 **------------------------------------------------------------**/
package middleware
...
var (
	labels = []string{"status", "uri", "method"}
...
func Prom(promOpts *PromOpts) server.HandlerFn {
	...
	return func(c *server.Context) {
		...
		lvs := []string{status, uri, method}

		isOk := promOpts.checkLabel(status, promOpts.ExcludeRegexStatus) &&
			promOpts.checkLabel(uri, promOpts.ExcludeRegexEndpoint) &&
...
func PromHandler(handler http.Handler) server.HandlerFn {
	return func(c *server.Context) {
...
*/
type RequestCounterURLLabelMappingFn func(c *server.Context) string
...
func NewPrometheus(subsystem string, customMetricsList ...[]*Metric) *Prometheus {
	...
	p := &Prometheus{
		...
		ReqCntURLLabelMappingFn: func(c *server.Context) string {
...
func (p *Prometheus) SetListenAddressWithRouter(listenAddress string, r *server.Engine) {
...
func (p *Prometheus) SetMetricsPath(e *server.Engine) {
...
//func (p *Prometheus) SetMetricsPathWithAuth(e *server.Engine, accounts gin.Accounts) {
//
//	if p.listenAddress != "" {
...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if _, err = client.Do(req); err != nil {
		log.Errorf("Error(%v) sending to push gateway", err)
	}
}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			log.Errorf("%s could not be registered in Prometheus err(%v)", metricDef.Name, err)
		}
		switch metricDef {
		case reqCnt:
...
func (p *Prometheus) Use(e *server.Engine) {
	e.Use(p.HandlerFunc())
	p.SetMetricsPath(e)
}

// UseWithAuth adds the middleware to a gin engine with BasicAuth.
//func (p *Prometheus) UseWithAuth(e *server.Engine, accounts gin.Accounts) {
//	e.Use(p.HandlerFunc())
//	p.SetMetricsPathWithAuth(e, accounts)
//}

// HandlerFunc defines handler function for middleware
func (p *Prometheus) HandlerFunc() server.HandlerFn {
	return func(c *server.Context) {
...
func prometheusHandler() server.HandlerFn {
	h := promhttp.Handler()
	return func(c *server.Context) {
